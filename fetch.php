...
foreach($settori as $settore) {
	...
	$url = "https://asn21.cineca.it/pubblico/miur/esito/".str_replace("/", "%252F",$settore)."/1/3";
	$page = get_page($url);
	...
	if($pubblicato) {
		...
		$new_found = "- " . date("d/m/Y") . ": " . $settore .
			" ([I Fascia](https://asn21.cineca.it/pubblico/miur/esito/" . str_replace("/", "%252F", $settore) . "/1/3), " .
			"[II Fascia](https://asn21.cineca.it/pubblico/miur/esito/" . str_replace("/", "%252F", $settore) . "/2/3))\n" .
			$new_found;
